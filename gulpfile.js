...
gulp.task('dev', ['build-dev'], function() {
  ...
  if (tars.config.notifyConfig.useNotify && env.NODE_ENV !== 'production' &&
   !env.DISABLE_NOTIFIER) {
    notify({
...
gulp.task('build-dev', function(cb) {
  ...
  runSequence(
    ...
    [
      ...
      'images:ut_make-svg-sprite'
      // TODO: make svg-icons system
    ],
    [
      // 'css:make-sprite-for-svg', 'css:make-fallback-for-svg',
      // 'css:make-sprite'
      'imgages:ut_make-png-sprites'
    ],
    [
      // 'css:compile-css', 'css:compile-css-for-ie8',
      'styles:ut_compile-to-css',
      // 'html:concat-modules-data',
      'html:ut_concat-modules-data',
      // 'js:move-separate', 'js:processing'
      'scripts:ut_move-vendors', 'scripts:ut_plugins', 'scripts:ut_processing'
    ],
    [
      // 'html:compile-templates',
      'html:ut_compile-to-html',
      // 'other:move-misc-files', 'other:move-fonts', 'other:move-assets',
      'other:ut_move-fonts', 'other:ut_move-assets'
      // 'images:move-content-img', 'images:move-plugins-img',
      // 'images:move-general-img'
    ],
    [
      'images:ut_make-svg-icons'
    ],
    cb
  );
});
...
gulp.task('build', function() {
  runSequence(
    'build-dev', [
      ...
      'html:ut_minify-html', 'images:ut_minify-images', 'other:ut_move-misc'
    ],
    'service:pre-build',
    [
      // 'css:compress-css'
      'styles:ut_compress-css'
    ],
    'service:zip-build',
    function() {
      console.log(gutil.colors.black.bold('\n--------------------------------' +
       '--------------------------'));
      tars.say(gutil.colors.green('✔') + gutil.colors.green.bold(' Build has' +
       'been created successfully!'));

      if (tars.config.useBuildVersioning) {
        tars.say(gutil.colors.white.bold('Build version is: ',
         tars.options.build.version));
      }
      console.log(gutil.colors.black.bold('----------------------------------' +
      '--------------------------\n'));
    }
